...
const CourseDetails = () => {
  ...
  const [isAlreadyEnroled,setIsAlreadyEnroled]= useState(false)
  ...
  return courseData ? (
    ...
    <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
     <div className='absolute top-0 left-0 w-full h-[500px] z-1 bg-gradient-to-b from-cyan-100/70 '></div>
       {/*left column*/}
        <div className='max-w-xl z-10 text-gray-500 '>
          <h1 className='md:text-[46px] text-[28px] font-semibold text-gray-800'> {courseData.courseTitle}</h1>
     ...
     <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
                 ...
                 <p className='text-sm'>Course by <span className='text-blue-600 underline'>Marvellous</span></p>
                <div className='pt-8 text-gray-800'>
                      ...
                      {courseData.courseContent.map((chapter,index)=>(
                        <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                          <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={()=>toggleSection(index)}>
                            ...
                            <p className='text-sm md:text-[19px]'>{chapter.chapterContent.length} lectures - {calculateChapterTime(chapter)}
                            </p>
                          </div>
                            ...
                            <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
                              {chapter.chapterContent.map((lecture,i)=>(
                                <li key={i} className='flex items-start gap-2 py-1'>
                                  ...
                                  <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-[19px]'>
                     ...
                     <div className='py-20 text-sm md:text-[19px]'>
      ...
      <div className='max-w-[424px] z-10 shadow-[0px 4px 15px 2px rgba(0,0,0,0.1)] rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
        ...
        <div className='p-5'>
          ...
          <div className='flex items-center text-sm md:text-[19px] gap-4 pt-2 md:pt-4 text-gray-500'>
          ...
          <button className='md:mt-6 mt-4 w-full py-3 rounded bg-blue-600 text-white font-medium'>{isAlreadyEnroled? 'Already Enrolled': 'Enroll Now'}</button>
        <div className='pt-6'>
          <p className='md:text-xl text-lg font-medium text-gray-800'>What is in the course?</p>
          <ul className='ml-4 pt-2 text-sm md:text-[19px] list-disc text-gray-500'>
